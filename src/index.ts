export default function transform(file, api, options) {
  ...
  // Transform `loader: 'raw-loader'` to `type: 'asset/source'`
  root.find(j.ObjectExpression).forEach((path) => {
      const loaderPropertyIndex = path.node.properties.findIndex(
          (prop) =>
              j.ObjectProperty.check(prop) &&
              j.Identifier.check(prop.key) &&
      ...
      if (loaderPropertyIndex !== -1) {
          // Remove `loader` property and add `type: 'asset/source'`
          path.node.properties.splice(
              loaderPropertyIndex,
              1,
              j.objectProperty(
